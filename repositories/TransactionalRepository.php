...
class TransactionalRepository
{
     ...
     * Save Transaction
     */
    public function saveTransaction(int $_employee_id, int $_leave_type, int $_leave_days, bool $_manager_approval_status): void
    {
        $query = "INSERT INTO save_transactional_details(employee_id, leave_days, leave_type_id, manager_approval_status)VALUES (?, ?, ?, ?)";
        $statement = $this->connection->prepare($query);
        $status = $_manager_approval_status ? 1 : 0;
        $statement->bind_param("iiii", $_employee_id, $_leave_days, $_leave_type, $status);
        $statement->execute();
    }
    ...
    {
        $leave_types = [];
        $query = "SELECT id, Leave_type_name FROM leave_type ORDER BY id";
        $result = $this->connection->query($query);
        while ($row = $result->fetch_assoc()) {
            $leave_types[] = ["id" => (int) $row["id"], "leave_type_name" => $row["Leave_type_name"]];
        }
    ...
    public function approveLeave(int $_transaction_id, bool $_approval_status): bool
    {
        ...
        $query = "UPDATE save_transactional_details SET manager_approval_status = ? WHERE id = ?";
        $statement = $this->connection->prepare($query);
        $status = $_approval_status ? 1 : 0;
        $statement->bind_param("ii", $status, $_transaction_id);
    ...
    {
        $transaction_history = [];
        $query = "SELECT std.id, ed.employee_id, ed.employee_name, std.leave_days, lt.Leave_type_name, std.manager_approval_status 
            FROM save_transactional_details std
            INNER JOIN employee_details ed ON std.employee_id = ed.id
            INNER JOIN leave_type lt ON std.leave_type_id = lt.id
            ORDER BY std.id";
        $result = $this->connection->query($query);
    ...
    {
        $transaction_history = [];
        $query = "SELECT std.id, ed.employee_id, ed.employee_name, std.leave_days, lt.Leave_type_name, std.manager_approval_status, std.created_at
            FROM save_transactional_details std
            INNER JOIN employee_details ed ON std.employee_id = ed.id
            INNER JOIN leave_type lt ON std.leave_type_id = lt.id
            WHERE DATE(std.created_at) = ?";
        $statement = $this->connection->prepare($query);
        $statement->bind_param("s", $_date);
        $statement->execute();
        $result = $statement->get_result();
        while ($row = $result->fetch_assoc()) {
            $transaction_history[] = [
